...
namespace Engine {
	...
	SolverPVTsim2D::SolverPVTsim2D(int Nx, int Ny, int numspec) :
	...
	std::vector<double> SolverPVTsim2D::mixture(double per1, double per2)
	{
	...
	void SolverPVTsim2D::set_initial_conditions()
	{
		...
		gamma = { 0.28, 0.541 , 1.8 } ; // { 0.241, 0.66 , 1.07 }
		...
		for (int i = 1; i <= mNx; i++)
		{
			for (int j = 1; j <= mNy; j++)
			{
				...
				if (((i - x_centre) * (i - x_centre) + (j - y_centre) * (j - y_centre)) <= (radius + delta) * (radius + delta) && 
					((i - x_centre) * (i - x_centre) + (j - y_centre) * (j - y_centre)) > radius * radius )
				{
					rhomulticomponent[0][i][j] = rho0liq + (rho0vap - rho0liq) * 0.2;
					rhomulticomponent[1][i][j] = rho1liq + (rho1vap - rho1liq) * 0.2;
					rhomulticomponent[2][i][j] = rho2liq + (rho2vap - rho2liq) * 0.2;
				}
				if (((i - x_centre) * (i - x_centre) + (j - y_centre) * (j - y_centre)) <= (radius + 2 * delta) * (radius + 2 * delta) &&
					((i - x_centre) * (i - x_centre) + (j - y_centre) * (j - y_centre)) > (radius + delta) * (radius + delta))
				{
					rhomulticomponent[0][i][j] = rho0liq + (rho0vap - rho0liq) * 0.4;
					rhomulticomponent[1][i][j] = rho1liq + (rho1vap - rho1liq) * 0.4;
					rhomulticomponent[2][i][j] = rho2liq + (rho2vap - rho2liq) * 0.4;
				}
				if (((i - x_centre) * (i - x_centre) + (j - y_centre) * (j - y_centre)) <= (radius + 3 * delta) * (radius + 3 * delta) &&
					((i - x_centre) * (i - x_centre) + (j - y_centre) * (j - y_centre)) > (radius + 2 * delta) * (radius + 2 * delta))
				{
					rhomulticomponent[0][i][j] = rho0liq + (rho0vap - rho0liq) * 0.6;
					rhomulticomponent[1][i][j] = rho1liq + (rho1vap - rho1liq) * 0.6;
					rhomulticomponent[2][i][j] = rho2liq + (rho2vap - rho2liq) * 0.6;
				}
				if (((i - x_centre) * (i - x_centre) + (j - y_centre) * (j - y_centre)) <= (radius + 4 * delta) * (radius + 4 * delta) &&
					((i - x_centre) * (i - x_centre) + (j - y_centre) * (j - y_centre)) > (radius + 3 * delta) * (radius + 3 * delta))
				{
					rhomulticomponent[0][i][j] = rho0liq + (rho0vap - rho0liq) * 0.8;
					rhomulticomponent[1][i][j] = rho1liq + (rho1vap - rho1liq) * 0.8;
					rhomulticomponent[2][i][j] = rho2liq + (rho2vap - rho2liq) * 0.8;
				}
			}
		}*/

		double x_centre = mNx / 2;
		double y_centre = mNy / 2;
		double a = 35;
		double b = 25;
		double delta = 1;
		for (int i = 1; i <= mNx; i++)
		{
			for (int j = 1; j <= mNy; j++)
			{
				...
				rhomulticomponent[1][i][j] = rho1vap;
				rhomulticomponent[2][i][j] = rho2vap;
				if (((i - x_centre) * (i - x_centre) / a / a + (j - y_centre) * (j - y_centre) / b / b) <= 1.)
				{
					rhomulticomponent[0][i][j] = rho0liq;
					rhomulticomponent[1][i][j] = rho1liq;
					rhomulticomponent[2][i][j] = rho2liq;
				}
				if (((i - x_centre) * (i - x_centre) / (a + delta) / (a + delta) + (j - y_centre) * (j - y_centre) / (b + delta) / (b + delta)) <= 1. &&
					((i - x_centre) * (i - x_centre) / a / a + (j - y_centre) * (j - y_centre) / b / b) > 1.)
				{
					rhomulticomponent[0][i][j] = rho0liq + (rho0vap - rho0liq) * 0.2;
					rhomulticomponent[1][i][j] = rho1liq + (rho1vap - rho1liq) * 0.2;
					rhomulticomponent[2][i][j] = rho2liq + (rho2vap - rho2liq) * 0.2;
				}
				if (((i - x_centre) * (i - x_centre) / (a + 2 * delta) / (a + 2 * delta) + (j - y_centre) * (j - y_centre) / (b + 2 * delta) / (b + 2 * delta)) <= 1. &&
					((i - x_centre) * (i - x_centre) / (a + delta) / (a + delta) + (j - y_centre) * (j - y_centre) / (b + delta) / (b + delta)) > 1.)
				{
					rhomulticomponent[0][i][j] = rho0liq + (rho0vap - rho0liq) * 0.4;
					rhomulticomponent[1][i][j] = rho1liq + (rho1vap - rho1liq) * 0.4;
					rhomulticomponent[2][i][j] = rho2liq + (rho2vap - rho2liq) * 0.4;
				}
				if (((i - x_centre) * (i - x_centre) / (a + 3 * delta) / (a + 3 * delta) + (j - y_centre) * (j - y_centre) / (b + 3 * delta) / (b + 3 * delta)) <= 1. &&
					((i - x_centre) * (i - x_centre) / (a + 2 * delta) / (a + 2 * delta) + (j - y_centre) * (j - y_centre) / (b + 2 * delta) / (b + 2 * delta)) > 1.)
				{
					rhomulticomponent[0][i][j] = rho0liq + (rho0vap - rho0liq) * 0.6;
					rhomulticomponent[1][i][j] = rho1liq + (rho1vap - rho1liq) * 0.6;
					rhomulticomponent[2][i][j] = rho2liq + (rho2vap - rho2liq) * 0.6;
				}
				if (((i - x_centre) * (i - x_centre) / (a + 4 * delta) / (a + 4 * delta) + (j - y_centre) * (j - y_centre) / (b + 4 * delta) / (b + 4 * delta)) <= 1. &&
					((i - x_centre) * (i - x_centre) / (a + 3 * delta) / (a + 3 * delta) + (j - y_centre) * (j - y_centre) / (b + 3 * delta) / (b + 3 * delta)) > 1.)
				{
					rhomulticomponent[0][i][j] = rho0liq + (rho0vap - rho0liq) * 0.8;
					rhomulticomponent[1][i][j] = rho1liq + (rho1vap - rho1liq) * 0.8;
					rhomulticomponent[2][i][j] = rho2liq + (rho2vap - rho2liq) * 0.8;
	...
	void SolverPVTsim2D::set_border_conditions()
	{
		/*Periodic*/
		/*for (int numspec = 0; numspec < number_of_species; numspec++)
		{
	...
	void SolverPVTsim2D::movement_step()
	{
		for (int numspec = 0; numspec < number_of_species; numspec++)
		{
	...
	void SolverPVTsim2D::calculate_moments()
	{
		ful_rho = 0;
		for (int i = 1; i <= mNx; i++) {
			for (int j = 1; j <= mNy; j++) {
	...
	void SolverPVTsim2D::calculate_pressure()
	{
		for (int i = 1; i <= mNx; i++) {
			for (int j = 1; j <= mNy; j++)
			{
	...
	void SolverPVTsim2D::calculate_force()
	{
		for (int i = 1; i <= mNx; i++) {
			for (int j = 1; j <= mNy; j++) {
				double force_x = 0;
				...
				for (int numspec = 0; numspec < number_of_species; numspec++) {
					...
					duy_force_spec[numspec][i][j] = force_y * gamma[numspec] / gamma_multiply_rho[i][j] - g * delta_t;
				}
			}
		}
	}
	...
	void SolverPVTsim2D::collision_step()
	{
		for (int numspec = 0; numspec < number_of_species; numspec++)
		{
	...
	void SolverPVTsim2D::check_rho()
	{
		...
		radius_of_droplet = sqrt(radius_of_droplet / 3.14159265);
	}


	void SolverPVTsim2D::eq_func(double rho, double ux, double uy, double* f_eq)
	{
	...
	void SolverPVTsim2D::eq_func_dimensional_moment(double rho, double ux, double uy, double* moment_eq)
	{
		moment_eq[0] = rho;
	...
	void SolverPVTsim2D::set_surface_force(int i, int j, double* surface_force)
	{
		double Qxx = 0.;
		double Qxy = 0.;
		double Qyy = 0.;
